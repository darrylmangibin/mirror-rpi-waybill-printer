...
4. Provide enough manifest detail, item status, export, queue-job status, and retry controls for operators to complete the manifest workflow without leaving the print-station frontend.
...
8. The operator reviews linked shipping bin items, filters items by tenant or sync status, exports the current item set as CSV for all items, selected items, or a tenant-scoped subset, manually syncs failed items when needed, and closes the manifest through `POST /shipping-manifests/:id/close` once no more items should be added.
