...
const DateField = ({ label, value }: { label: string; value: string | null }) => {
...
const ErrorState = ({ onBack }: { onBack: () => void }) => (
  <div className="rounded-2xl border border-rose-200 bg-white px-6 py-10 text-center shadow-sm">
    ...
    <h2 className="mt-4 text-lg font-semibold text-slate-900">Manifest not available</h2>
            ...
            <div className="space-y-6">
              <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                ...
                <div className="grid gap-4 border-t border-slate-100 bg-white px-6 py-5 sm:grid-cols-2 xl:grid-cols-4">
                  <DetailField label="Receiver" value={manifest.receiver_name} />
                  <DetailField label="Vehicle Plate" value={manifest.vehicle_plate_number} mono />
                  <DetailField label="Loaded Orders" value={manifest.loaded_orders_count ?? 0} />
                  <DetailField label="Generated By" value={manifest.generated_by_username ?? manifest.generated_by_id} />
                </div>
              </section>
              ...
              <div className="grid gap-6 xl:grid-cols-[1.35fr_0.95fr]">
                <div className="space-y-6">
                  ...
                  >
                    <DetailField label="Carrier" value={manifest.shipping_carrier} />
                    <DetailField label="Manifest Path" value={manifest.manifest_path} mono />
                    <DetailField label="Receiver Name" value={manifest.receiver_name} />
                    <DetailField label="Vehicle Plate Number" value={manifest.vehicle_plate_number} mono />
                    <DateField label="Loading Started" value={manifest.loading_started_at} />
                    <DateField label="Loaded At" value={manifest.loaded_at} />
                    <DateField label="Delivery Completed" value={manifest.delivery_completed_at} />
                    <DetailField label="Tenant ID" value={manifest.tenant_id} mono />
                  </SectionCard>
                ...
                <div className="space-y-6">
                  ...
                  >
                    ...
                    <DetailField label="Manifest Code" value={manifest.manifest_code} mono />
                    <DetailField label="Status" value={formatLabel(manifest.status)} />
                    <DetailField label="Generation Type" value={formatLabel(manifest.generation_type)} />
                    <DetailField label="Generated By ID" value={manifest.generated_by_id} mono />
                    <DetailField label="Generated By Username" value={manifest.generated_by_username} />
                  </SectionCard>
                  ...
                  >
                    <DetailField label="Current Status" value={formatLabel(manifest.status)} />
                    <DetailField label="Orders in Manifest" value={manifest.loaded_orders_count ?? 0} />
                    <DetailField label="Manifest Path Available" value={manifest.manifest_path ? "Yes" : "No"} />
                    <DetailField label="Metadata Available" value={manifest.meta_data ? "Yes" : "No"} />
                  </SectionCard>
                </div>
              </div>
            </div>
