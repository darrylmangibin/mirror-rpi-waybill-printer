...
interface CancelPrintConfirmationDialogProps {
	waybill: WaybillPrint | null;
	open: boolean;
	onOpenChange: (open: boolean) => void;
	onConfirm?: (waybill: WaybillPrint) => Promise<void>;
	isLoading?: boolean;
}

export const CancelPrintConfirmationDialog = ({
	waybill,
	open,
	onOpenChange,
	onConfirm,
	isLoading = false,
}: CancelPrintConfirmationDialogProps) => {
	if (!waybill) return null;

	const handleConfirm = async () => {
		if (onConfirm) {
			await onConfirm(waybill);
			onOpenChange(false);
		}
	};
				...
				<div className='px-5 py-3 space-y-2.5'>
					...
					<div className='bg-amber-50 rounded-md p-2.5 border border-amber-200'>
						...
						<ul className='text-xs text-amber-700 space-y-0.5 list-disc list-inside'>
							<li>Cancel the active print job on CUPS</li>
					...
					<div className='flex items-center justify-end gap-2 w-full bg-gray-50 border-t border-gray-200 py-3 px-5 rounded-b-lg'>
						...
						<PrimaryButton
							...
							<span className='flex items-center gap-1.5'>
								...
								{isLoading ? 'Cancelling...' : 'Cancel Print Job'}
							</span>
						</PrimaryButton>
					</div>
				</DialogFooter>
