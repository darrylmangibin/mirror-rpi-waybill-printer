...
const Home = () => {
  ...
  const [createWaybillDialogOpen, setCreateWaybillDialogOpen] =
  ...
  const { mutateAsync: cancelPrintAsync, isPending: isCancelling } =
    useCancelPrintWaybill();
  const {
    mutateAsync: deleteWaybillAsync,
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <>
            <EditWaybillPrintDialog
              ...
              onSuccess={async () => {
                ...
                const updatedWaybill = waybills.find(
                  (w) => w.id === selectedWaybill?.id
                );
                if (updatedWaybill) {
        ...
        <CreateWaybillPrintDialog
          open={createWaybillDialogOpen}
          onOpenChange={setCreateWaybillDialogOpen}
          onSubmit={async () => {
        ...
        <CleanupWaybillsDialog
          open={cleanupDialogOpen}
          onOpenChange={setCleanupDialogOpen}
          onSuccess={async () => {
            // Enable polling to watch for updates after cleanup
            setIsPolling(true);
            // SSE will notify of changes immediately
