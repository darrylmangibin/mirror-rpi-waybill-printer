...
echo -e "${YELLOW}Installing system dependencies (Ghostscript for PDF conversion)...${NC}"
sudo apt update
sudo apt install -y ghostscript poppler-utils
echo -e "${GREEN}✅ System dependencies installed${NC}"

# Install USB support for thermal printer (python-escpos)
echo -e "${YELLOW}Installing USB support for thermal printer...${NC}"
sudo apt install -y libusb-1.0-0 libusb-dev
echo -e "${GREEN}✅ USB support installed${NC}"

# Note about thermal printer
echo -e "${BLUE}ℹ️  XPrinter XP-410B Thermal Printer Setup${NC}"
echo -e "${BLUE}   - Connection: Direct USB (ESC/POS protocol)${NC}"
echo -e "${BLUE}   - No CUPS drivers needed${NC}"
echo -e "${BLUE}   - Ensure printer is connected via USB and powered on${NC}"
echo -e "${BLUE}   - Verify with: lsusb | grep -i xprinter${NC}"
