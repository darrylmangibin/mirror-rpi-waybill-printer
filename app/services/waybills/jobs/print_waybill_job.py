    ...
    while True:
        try:
            ...
            with app.app_context():
                ...
                try:
                    from app.services.waybills.actions.PrintWaybillAction import (
                        PrintWaybillAction,
                    )
                    ...
                    logger.info(
                        f"[PRINT COMPLETE] Invoice: {invoice}, Status: {result.get('status')}"
                    )

                except Exception as e:
                    logger.error(
                        f"[PRINT ERROR] Invoice: {invoice}: {str(e)}", exc_info=True
                    )
...
def start_print_workers(num_workers=1):
    ...
    for i in range(num_workers):
        thread = threading.Thread(
            target=print_worker, daemon=True, name=f"WaybillPrintWorker-{i}"
        )
        thread.start()
