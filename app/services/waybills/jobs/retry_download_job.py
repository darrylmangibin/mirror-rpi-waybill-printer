    ...
    while True:
        try:
            ...
            with app.app_context():
                ...
                logger.info(f"[RETRY STARTED] Invoice: {invoice}, Marketplace: {waybill.marketplace}, Attempt: {current_retry}/{MAX_RETRIES}")
                ...
                try:
                    ...
                    if result.get('status') == 'success':
                        logger.info(f"[RETRY SUCCESS] Invoice: {invoice}, Marketplace: {waybill.marketplace}, Attempt: {current_retry}/{MAX_RETRIES}")
                    else:
                        logger.warning(f"[RETRY FAILED] Invoice: {invoice}, Marketplace: {waybill.marketplace}, Attempt: {current_retry}/{MAX_RETRIES}: {result.get('message')}")
                    
                except Exception as e:
                    logger.error(f"[RETRY ERROR] Invoice: {invoice}, Marketplace: {waybill.marketplace}, Attempt: {current_retry}/{MAX_RETRIES}: {str(e)}", exc_info=True)
...
def start_retry_workers(num_workers=1):
    ...
    for i in range(num_workers):
        thread = threading.Thread(target=retry_worker, daemon=True, name=f"WaybillRetryWorker-{i}")
        thread.start()
