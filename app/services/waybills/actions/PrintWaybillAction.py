...
class PrintWaybillAction:
    ...
    def __call__(self, waybill_print: WaybillPrint) -> dict:
        ...
        try:
            ...
            # NOTE: CRON job will automatically monitor this print job (no queue needed)
            if result.get('status') == 'success' and result.get('data'):
                cups_job_id = result['data'].get('job_id')
                logger.info(f"[PRINT SUBMITTED] Invoice: {invoice_number}, JobID: {cups_job_id} - Will be monitored by CRON job")
